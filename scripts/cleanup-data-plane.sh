...
AWS_ROLE="Admin"
CLUSTER_ARN="FizzyDrPepper-EcsCluster-beta-us-east-1-cell1-ClusterEB0386A7-hrV96abetIpU"
CODE_DEPLOY_APPLICATION="FizzyDrPepper-CodeDeploy-beta-us-east-1-cell1-CodeDeployApplicationE587C27C-1IHEUWX7XA7YG"
DEFAULT_VPC="vpc-4b0abe36"
BASE_VPC="vpc-0c62aec1df1c7368b"

ada credentials update --account=$AWS_ACCOUNT --provider=isengard --role=$AWS_ROLE --once

AWS="aws --no-paginate --no-cli-pager --no-cli-auto-prompt --region $AWS_REGION "
...
for DEPLOYMENT_GROUP in "${DEPLOYMENT_GROUP_LIST[@]}"; do
...
OPENSEARCH_VPCE_LIST=($($AWS opensearch list-vpc-endpoints | jq '.VpcEndpointSummaryList | .[] | .VpcEndpointId' | tr -d '"'))
for OPENSEARCH_VPCE in "${OPENSEARCH_VPCE_LIST[@]}"; do
    $AWS opensearch delete-vpc-endpoint --vpc-endpoint-id $OPENSEARCH_VPCE
done
...
for VPCE_SERVICE in "${VPCE_SERVICE_LIST[@]}"; do
    ...
    for VPCE_ENDPOINT in "${VPCE_ENDPOINT_LIST[@]}"; do
...
VPCE_LIST=($(aws ec2 describe-vpc-endpoints | jq '.VpcEndpoints[].VpcEndpointId' | tr -d '"'))
for VPCE in "${VPCE_LIST[@]}"; do
...
for NLB in "${NLB_LIST[@]}"; do
    ...
    for LISTENER in "${LISTENER_LIST[@]}"; do
...
TASK_DEFINITION_LIST=($($AWS ecs list-task-definitions | jq '.taskDefinitionArns[] ' | tr -d '"'))
for TASK_DEFINITION in "${TASK_DEFINITION_LIST[@]}"; do
    echo "deregistering $TASK_DEFINITION"
    $AWS ecs deregister-task-definition --task-definition "$TASK_DEFINITION"
    echo "deleting $TASK_DEFINITION"
    $AWS ecs delete-task-definitions --task-definitions "$TASK_DEFINITION"
done
...
for ENI_ATTACHMENT in "${ENI_ATTACHMENT_LIST[@]}"; do
...
for SECURITY_GROUP in "${SECURITY_GROUP_LIST[@]}"; do
...
EIP_ASSOCIATION_LIST=($($AWS ec2 describe-addresses | jq '.Addresses[] | .AssociationId | select( . != null )' | tr -d '"'))
...
EIP_ALLOCATION_LIST=($($AWS ec2 describe-addresses | jq '.Addresses[] | .AllocationId | select( . != null )' | tr -d '"'))
